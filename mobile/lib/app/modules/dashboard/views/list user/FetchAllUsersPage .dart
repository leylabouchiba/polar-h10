    ...
    return Scaffold(
      ...
      body: GetBuilder<FetchAllUsersController>(
        ...
        builder: (controller) {
          ...
          final filteredUsers = controller.users.where((user) => user.isCoach == 'User').toList();

          if (filteredUsers.isEmpty) {
            return Center(
              child: CircularProgressIndicator(), // Affiche un indicateur de chargement si la liste des utilisateurs filtrés est vide
            );
          } else {
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  elevation: 3, // Ajoute une élévation à la carte pour un aspect tridimensionnel
                  margin: EdgeInsets.symmetric(vertical: 8, horizontal: 16), // Marge autour de la carte
                  child: ListTile(
                    contentPadding: EdgeInsets.all(16), // Ajoute du rembourrage à l'intérieur de la liste
                    leading: CircleAvatar(
                      radius: 30, // Augmente la taille du cercle pour l'image de profil
                      backgroundImage: NetworkImage(user.photo ?? ''), // Remplacez par l'URL de l'image de l'utilisateur
                    ),
                    title: Text(
                      user.firstName ?? '',
                      style: TextStyle(
                        fontWeight: FontWeight.bold, // Met en gras le prénom de l'utilisateur
                      ),
                    ),
                    subtitle: Text(
                      user.email ?? '',
                      style: TextStyle(
                        fontStyle: FontStyle.italic, // Ajoute un style italique à l'email de l'utilisateur
                      ),
                    ),
